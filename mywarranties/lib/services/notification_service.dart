...
class NotificationService {
  ...
  Future<void> checkWarrantiesExpiringSoon() async {
    ...
    try {
      ...
      // Get products that might expire within the next 30 days
      // Calculate the cutoff date (30 days from now)
      final now = DateTime.now();
      final cutoffDate = now.add(const Duration(days: 30));
      
      // Query products more efficiently - we'll filter in code since Firestore
      // doesn't support complex date calculations on stored warranty periods
      ...
      // Filter products that have expiry dates within our range
      final relevantProducts = <QueryDocumentSnapshot<Map<String, dynamic>>>[];
      ...
      for (var doc in productsSnapshot.docs) {
        ...
        // Only process products that expire within the next 30 days or have already expired
        if (expiryDate != null && expiryDate.isBefore(cutoffDate)) {
          relevantProducts.add(doc);
        }
      }
      
      print('📊 Filtered ${relevantProducts.length} relevant products from ${productsSnapshot.docs.length} total products');
      
      for (var doc in relevantProducts) {
        final data = doc.data();
